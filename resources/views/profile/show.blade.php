...
@section('content')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div class="bg-gradient-to-br from-blue-100 to-white p-4 rounded-lg shadow-lg">
                ...
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-sm text-gray-600">{{ auth()->user()->email ?? 'john.doe@example.com' }}</p>
            ...
            <div class="bg-gradient-to-br from-green-100 to-white p-4 rounded-lg shadow-lg">
                ...
                <p><strong>Name:</strong> {{ auth()->user()->name ?? 'Null' }}</p>
                <p><strong>Phone:</strong> {{ $userDetail->phone ?? 'Null' }}</p>
                <p><strong>Gender:</strong> {{ $userDetail->gender ?? 'Null' }}</p>
                <p><strong>Date of Birth:</strong> {{ $userDetail->date_of_birth ? $userDetail->date_of_birth->format('Y-m-d') : 'null' }}</p>
                <p><strong>Address:</strong> {{ $userDetail->address ?? 'Null' }}</p>
            ...
            <div class="bg-gradient-to-br from-purple-100 to-white p-4 rounded-lg shadow-lg">
                ...
                <p><strong>Name:</strong> {{ $userDetail->next_of_kin_name ?? 'Jane Doe' }}</p>
                <p><strong>Email:</strong> {{ $userDetail->next_of_kin_email ?? 'jane.doe@example.com' }}</p>
                <p><strong>Phone:</strong> {{ $userDetail->next_of_kin_phone ?? '+1 234 567 8901' }}</p>
                <p><strong>Relationship:</strong> {{ $userDetail->next_of_kin_relationship ?? 'Sister' }}</p>
    ...
    <div class="md:ml-64 lg:ml-0 bg-gradient-to-br from-red-100 to-white p-6 rounded-lg shadow">
        ...
        <p><strong>Height:</strong> {{ $medicalInfo->height ?? '175 cm' }}</p>
        <p><strong>Blood Group:</strong> {{ $medicalInfo->blood_group ?? 'A+' }}</p>
        <p><strong>Genotype:</strong> {{ $medicalInfo->genotype ?? 'AA' }}</p>
        <p><strong>Known Allergies:</strong> {{ $medicalInfo->known_allergies ?? 'Penicillin' }}</p>
        <p><strong>Health Issues:</strong> {{ $medicalInfo->health_issues ?? 'None' }}</p>
